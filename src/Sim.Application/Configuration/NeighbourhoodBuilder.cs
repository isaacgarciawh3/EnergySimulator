...
public static class NeighbourhoodBuilder
{
    ...
    public static Neighbourhood Build(SimulationConfiguration configuration)
    {
        ...
        for (var i = 1; i <= Neighbourhood.RequiredHouses; i++)
        {
            ...
            {
                new($"{id}/base", id, AssetType.BaseLoad, 0.2 + 0.4 * DeterministicNoise.Sample(seed, Baseline, i)),
            };
            if (DeterministicNoise.Sample(seed, PvDraw, i) < configuration.PvShare)
                assets.Add(new Asset($"{id}/pv", id, AssetType.Pv, 3.0 + 5.0 * DeterministicNoise.Sample(seed, PvSize, i)));
            if (DeterministicNoise.Sample(seed, HpDraw, i) < configuration.HeatPumpShare)
                assets.Add(new Asset($"{id}/heat-pump", id, AssetType.HeatPump, 3.0,
            ...
            if (DeterministicNoise.Sample(seed, EvDraw, i) < configuration.HomeEvShare)
                assets.Add(new Asset($"{id}/ev-charger", id, AssetType.HomeEvCharger, 7.4));
