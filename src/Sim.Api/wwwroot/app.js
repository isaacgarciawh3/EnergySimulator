...
function drawChart(points) {
  ...
  svg.innerHTML = `
    ...
    <path d="${line(p => p.netWithoutBatteryKw || 0)}" fill="none" stroke="#8794b0" stroke-width="1.6" stroke-dasharray="5 4"/>
...
/** State-of-charge trace inside the battery panel. */
function drawSoc(points) {
  const svg = $('socChart'), W = 400, H = 70;
  if (!svg || !points || !points.length) { if (svg) svg.innerHTML = ''; return; }
  const x = i => (i / Math.max(1, points.length - 1)) * W;
  const y = v => H - 4 - (Math.max(0, Math.min(100, v)) / 100) * (H - 10);
  const d = points.map((p, i) => `${i ? 'L' : 'M'} ${x(i).toFixed(1)} ${y(p.socPercent || 0).toFixed(1)}`).join(' ');
  svg.innerHTML = `
    <line x1="0" y1="${y(100)}" x2="${W}" y2="${y(100)}" stroke="#1f2942"/>
    <line x1="0" y1="${y(50)}" x2="${W}" y2="${y(50)}" stroke="#1f2942"/>
    <path d="${d} L ${W} ${H} L 0 ${H} Z" fill="rgba(61,220,151,.14)"/>
    <path d="${d}" fill="none" stroke="#3ddc97" stroke-width="1.8"/>
    <text x="4" y="10" fill="#8794b0" font-size="9">100%</text>`;
}
...
function render(s) {
  ...
  drawSoc(s.last24Hours);

  // The two peak figures measure DIFFERENT things and are labelled as such:
  // the banner pair is cumulative since simulation start, the flattening figure
  // is within the visible 24h window. Never present one as the other.
  const cut = s.peakWithoutBatteryKw - s.peakWithBatteryKw;
  $('peakNo').textContent = s.peakWithoutBatteryKw.toFixed(1);
  $('peakYes').textContent = s.peakWithBatteryKw.toFixed(1);
  $('peakCut').textContent = cut.toFixed(1);
  $('peakCutPct').textContent = s.peakWithoutBatteryKw > 0
    ? `${(100 * cut / s.peakWithoutBatteryKw).toFixed(1)}% lower, since start` : 'since start';

  const w = s.last24Hours || [];
  if (w.length) {
    const maxNo = Math.max(...w.map(p => p.netWithoutBatteryKw || 0));
    const maxYes = Math.max(...w.map(p => p.netKw));
    $('windowCut').textContent = maxNo > 0 ? (100 * (maxNo - maxYes) / maxNo).toFixed(1) : '0.0';
  }

  const b = s.battery;
  if (b) {
    $('batteryPanel').style.display = '';
    $('batMode').textContent = b.mode;
    $('batMode').className = 'bat-mode ' + b.mode;
    $('batKw').textContent = b.powerKw.toFixed(1);
    $('socFill').style.width = b.stateOfChargePercent + '%';
    $('socText').textContent = `${b.stateOfChargePercent.toFixed(1)}%  ·  ${b.stateOfChargeKwh.toFixed(0)} / ${b.capacityKwh.toFixed(0)} kWh`;
    $('batIn').textContent = b.chargedKwh.toFixed(0);
    $('batOut').textContent = b.dischargedKwh.toFixed(0);
    $('stratName').textContent = b.strategy;
  } else {
  ...
  const rows = s.meters.filter(m =>
    !filter || m.meterId.toLowerCase().includes(filter) || m.category.toLowerCase().includes(filter));
...
async function poll() {
  ...
  catch { /* transient while the simulation is being rebuilt */ }
}
