...
const COLOR = { s1: '#3987e5', s2: '#d95926', s3: '#199e70', grid: '#1f2942', axis: '#2b3category', ink: '#8794b0', surface: '#121829' };
COLOR.axis = '#2b3a5c';
...
function renderPeaks(s, points) {
  ...
  $('battStateBadge').textContent = hasBattery
    ? `${f(s.battery.capacityKwh, 0)} kWh / ${f(s.batteryMaxPowerKw || (s.battery && s.battery.capacityKwh && 0), 0) && ''}`.trim() && `${f(s.battery.capacityKwh, 0)} kWh battery installed`
...
function batteryLayout(has) {
  ...
  $('batteryBody').innerHTML = has ? `
    ...
    <div class="batt-facts">
      ...
      <div class="fact"><span class="k">Max power</span><span class="v"><b id="bMax">0</b><i>kW</i></span></div>
...
function renderBattery(s, points) {
  ...
  $('bMax').textContent = f(s.batteryMaxPowerKw != null ? s.batteryMaxPowerKw : maxSeen(points), 0);
...
/** The snapshot does not carry the battery power limit, so infer it from the window. */
function maxSeen(points) {
  return points.length ? Math.max(...points.map(p => Math.abs(num(p.batteryKw)))) : 0;
}
