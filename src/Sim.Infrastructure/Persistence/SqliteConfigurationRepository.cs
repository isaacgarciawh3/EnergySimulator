...
/// <summary>
/// SQLite adapter for <see cref="ISimulationConfigurationRepository"/>.
///
/// It stores and retrieves. It does NOT decide what should exist when nothing
/// has been stored - that policy belongs to the application layer, which reads
/// the defaults from the configuration file (ADR-0012).
/// </summary>
public sealed class SqliteConfigurationRepository(SqliteConnectionFactory factory) : ISimulationConfigurationRepository
{
    public SimulationConfiguration? Find()
    {
        using var connection = factory.Open();
        using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT seed, start_instant, tick_minutes, ticks_per_second, pv_share, heat_pump_share,
                   home_ev_share, battery_capacity_kwh, battery_max_power_kw, battery_efficiency,
                   peak_threshold_kw, battery_enabled
            FROM simulation_configuration WHERE id = 1;
            """;
    ...
    public bool Exists()
    {
        using var connection = factory.Open();
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(*) FROM simulation_configuration WHERE id = 1;";
    ...
    public void Clear()
    {
        using var connection = factory.Open();
        using var command = connection.CreateCommand();
        command.CommandText = "DELETE FROM simulation_configuration WHERE id = 1;";
    ...
    public void Save(SimulationConfiguration configuration)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO simulation_configuration (id, seed, start_instant, tick_minutes, ticks_per_second,
                pv_share, heat_pump_share, home_ev_share,
